// content.js — WPS 365 Claude Code 浮动面板
(function () {
  ...
  document.addEventListener("mouseup", () => {
    if (panel.classList.contains("cc-visible")) setTimeout(requestSelection, 50);
  }, true);
  document.addEventListener("keyup", (e) => {
    if (panel.classList.contains("cc-visible") && (e.shiftKey || e.key === "Shift")) requestSelection();
  }, true);
  ...
  function updateSelectionBar() {
    if (cachedSelection) {
      ...
      selectionBar.querySelector(".cc-sel-clear").addEventListener("click", () => {
        cachedSelection = "";
        updateSelectionBar();
      });
    } else {
  ...
  function updateStatus(el, msg) {
    const last = el.querySelector('.cc-step[data-type="status"]');
    if (last) last.querySelector(".cc-step-text").textContent = msg;
    else addStep(el, "status", msg);
  }

  function clearSteps(el, type) {
    el.querySelectorAll(`.cc-step[data-type="${type}"]`).forEach(n => n.remove());
  }
  ...
  function renderMarkdown(text) {
    ...
    p = esc(p);
    p = p.replace(/%%CB(\d+)%%/g, (_, i) => cb[i]);
    p = p.replace(/%%IC(\d+)%%/g, (_, i) => ic[i]);
    ...
    p = p.replace(/^(\|.+\|)\n(\|[\s:|-]+\|)\n((?:\|.+\|\n?)+)/gm, (_, h, _s, b) => {
      const hs = h.split("|").filter(Boolean).map(c => c.trim());
      const rs = b.trim().split("\n").map(r => r.split("|").filter(Boolean).map(c => c.trim()));
      let t = "<table><thead><tr>" + hs.map(c => `<th>${c}</th>`).join("") + "</tr></thead><tbody>";
    ...
    p = blocks.map(b => { b = b.trim(); if (!b) return ""; if (/^<(h[1-6]|pre|ul|ol|table|blockquote|hr)/.test(b)) return b; return `<p>${b.replace(/\n/g, "<br/>")}</p>`; }).join("\n");
  ...
  async function send() {
    ...
    function flushTextToStep() {
      ...
      const short = accumulated.trim().length > 120 ? accumulated.trim().slice(0, 120) + "…" : accumulated.trim();
      const el = document.createElement("div");
      el.className = "cc-thought";
    ...
    function collapseActivity() {
      ...
      const details = activityEl.innerHTML;
      activityEl.innerHTML = "";
      const summary = document.createElement("div");
      summary.className = "cc-activity-summary";
      summary.innerHTML = `<span class="cc-summary-toggle">▶</span> 已完成 · ${stepCount}步 · ${timeStr}`;
      const detailsWrap = document.createElement("div");
      detailsWrap.className = "cc-activity-details";
      ...
      activityEl.appendChild(detailsWrap);
    }
    ...
    function handleEvent(ev) {
      switch (ev.type) {
        ...
        case "thinking_start":
          clearSteps(activityEl, "status");
          // Replace any existing thinking step instead of adding another
          if (!activityEl.querySelector('.cc-step[data-type="thinking"]')) {
            addStep(activityEl, "thinking", "思考中…");
          }
          break;
        ...
        case "thinking_done": {
          // Just remove the thinking indicator silently — no "思考完成" clutter
          const s = activityEl.querySelector('.cc-step[data-type="thinking"]');
          if (s) s.remove();
          break;
        }
        ...
        case "delta":
          if (!gotContent) {
            gotContent = true;
            clearSteps(activityEl, "status");
            clearSteps(activityEl, "thinking");
          }
          accumulated += ev.text;
          scheduleRender();
          break;
        ...
        case "tool_start": {
          ...
          currentToolStep = addStep(activityEl, "tool_start", label(ev.name));
          break;
        }
        ...
        case "tool_detail":
          if (currentToolStep && ev.text) {
            ...
            // Shorten file paths: show only filename
            let displayText = ev.text;
            if (displayText.includes("/")) {
              displayText = displayText.replace(/\/[\w./-]+\/([\w.-]+)/g, "…/$1");
            }
            d.textContent = displayText.length > 80 ? displayText.slice(0, 80) + "…" : displayText;
            scrollToBottom();
          }
          break;
        ...
        case "tool_result":
          if (currentToolStep) {
            ...
            ic.className = "cc-step-icon";
            ic.textContent = ev.is_error ? "❌" : "✅";
        ...
        case "done":
          ...
          if (renderFrame) { cancelAnimationFrame(renderFrame); renderFrame = null; }
          // Collapse activity into summary
          collapseActivity();
          replyEl.innerHTML = renderMarkdown(ev.result || accumulated);
          scrollToBottom();
          break;
        ...
        case "error": {
          ...
          if (isTimeout) {
            ...
            replyEl.querySelector(".cc-error").appendChild(btn);
          }
          scrollToBottom();
          break;
        }
  ...
  // ========== DRAG ==========
  (function () {
    const hdr = panel.querySelector(".cc-header");
    let d = false, sx, sy, sl, st;
    hdr.addEventListener("mousedown", (e) => {
      if (e.target.closest(".cc-header-btn")) return;
      d = true; const r = panel.getBoundingClientRect();
